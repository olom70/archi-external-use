...
from common import log_function_call
...
@log_function_call
def importFL(MAIN_FOLDER: str, OUTPUT: str, FUNCTIONLIST_NAME: str, TAB: str, YEDFILEPREFIX: str, ARCHIPREFIX: str) -> bool: 
    ...
    def writelink(LevelID: str, columnToLink: str, valueOfTheCell: str ) -> None:
        ...
        try:
            ...
            found = False
        if found:
            writeLine(csvutil.initRelations(ID=LevelID, Type=linkType, Source=source, Target=target), 5)
    ...
    if not os.path.isfile(INPUT):
        ...
        for items in zip(colAcolId, colBmacroprocess, colCfunctionfuture, colDkeyword, colEmarketid, colFsubfunctionlist, colGfuturesubfunctiongeneraldescription, colHuserstoryinformation, colIopoldnpnew, colJsubfunctfutureyntd, colKsuitabilityofthesitoday, colLcollabor, colMprodcom, colNlink, colOapo, colPfms, colQecc, colRcape, colSordermax, colTrank2, colUassessgo, colVcpp, colWpsv, colXsnc, colYmoldgo, colZassetgo,
            ...
            if processLine:
                ...
                Level2ID = getcleanedID(c, 2)
                Level3ID = getcleanedID(f, 3)
                
                #Process Level1 ###############################################################
                if Level1ID not in l_alreadyAdded:
                    try:
                        graph.write_graph(MAIN_FOLDER + os.path.sep + OUTPUT + os.path.sep + l_alreadyAdded[len(l_alreadyAdded)-1], pretty_print=True)
                    except NameError:
                        pass
                    graph = openYed()
                    ...
                    graph.add_node(Level1ID, label=Name, font_size=cfl.YedProperties.L1FONTSIZE, font_style=cfl.YedProperties.L1FONTSTYLE, width=cfl.YedProperties.WIDTH, shape_fill=cfl.YedProperties.L1COLOR)
                ...
                if Level2ID not in l_alreadyAdded:
                    ...
                    writeLine(csvutil.initElements(ID=Level2ID, Type=cfl.ArchiConcepts.BUSINESSFUNCTION, Name=Name, Documentation=Documentation), 1)
                    ...
                    writeLine(csvutil.initRelations(ID=Level2ID, Type=cfl.ArchiConcepts.COMPOSITIONRELATION, Source=Level1ID, Target=Level2ID), 5)
                    graph.add_node(Level2ID, label=Name, font_size=cfl.YedProperties.L2FONTSIZE, font_style=cfl.YedProperties.L2FONTSTYLE, width=cfl.YedProperties.WIDTH, shape_fill=cfl.YedProperties.L2COLOR)
                ...
                if Level3ID not in l_alreadyAdded:
                    ...
                    writeLine(csvutil.initElements(ID=Level3ID, Type=cfl.ArchiConcepts.BUSINESSFUNCTION, Name=Name, Documentation=Documentation), 1)
                    ...
                    writeLine(csvutil.initRelations(ID=Level3ID, Type=cfl.ArchiConcepts.COMPOSITIONRELATION, Source=Level2ID, Target=Level3ID), 5)
                    everythingwascool = False if not writelink(LevelID=Level3ID, columnToLink='I', valueOfTheCell=i) else everythingwascool

                    graph.add_node(Level3ID, label=Name, font_size=cfl.YedProperties.L3FONTSIZE, font_style=cfl.YedProperties.L3FONTSTYLE, width=cfl.YedProperties.WIDTH, shape_fill=cfl.YedProperties.L3COLOR,
